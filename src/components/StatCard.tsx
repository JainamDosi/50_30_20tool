...
export function StatCard({ label, value, icon, subtext, trend, status, statusColor, mono }: StatCardProps) {
    return (
        <motion.div whileHover={{ y: -6 }} className="stat-card !p-8 group shadow-2xl">
            <div className="flex items-center justify-between mb-8">
                <span className="text-zinc-500 text-[10px] font-black font-mono uppercase tracking-[0.3em]">{label}</span>
                <div className="p-2.5 bg-black rounded-xl border border-white/5 shadow-inner transition-colors group-hover:border-white/10">{icon}</div>
            </div>
            <div className="flex items-baseline gap-3">
                <span className={cn("text-4xl font-black tracking-tighter text-white", mono && "font-mono")}>{value}</span>
                {trend && (
