    ...
    return (
        ...
        >
            ...
            <motion.div
                ...
                className="text-center space-y-10 max-w-5xl z-10"
            >
                <div className="inline-flex items-center gap-3 px-5 py-2.5 rounded-full glass border-2! border-emerald-500! text-emerald-400 text-[10px] font-black tracking-[0.4em] uppercase shadow-[0_0_40px_rgba(16,185,129,0.1)]">
                    <Zap className="w-3 h-3 fill-emerald-400 animate-pulse" />
                    Ready
                </div>

                <h1 className="text-7xl md:text-9xl font-black font-sans tracking-tight leading-[0.85] text-white">
                    BETTER <br />
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 via-sky-400 to-amber-500">BUDGET.</span>
                </h1>

                <p className="text-zinc-500 text-xl md:text-2xl font-medium max-w-3xl mx-auto leading-relaxed tracking-tight">
                ...
                <div className="flex flex-col sm:flex-row items-center justify-center gap-8 pt-12">
                    <motion.button
                        ...
                        whileTap={{ scale: 0.95 }}
                        onClick={handleEnter}
                        className="group flex items-center gap-4 px-12 py-6 bg-white text-black text-[11px] font-black uppercase tracking-[0.4em] rounded-[2rem] transition-all"
                    >
                        Open Dashboard
                        <ArrowRight className="w-4 h-4 group-hover:translate-x-2 transition-transform" />
                    </motion.button>

                    <div className="flex items-center gap-10 text-[10px] font-black text-zinc-700 uppercase tracking-[0.3em]">
                        <div className="flex flex-col items-center gap-3 group">
                            <Layers className="w-6 h-6 text-emerald-500/30 group-hover:text-emerald-400 transition-colors" />
                            Smart Tracking
                        </div>
                        <div className="flex flex-col items-center gap-3 group">
                            <Gauge className="w-6 h-6 text-sky-500/30 group-hover:text-sky-400 transition-colors" />
                            Save More
                        </div>
                    </div>
                </div>
